...
export const checkStatus = async (req, res) => {
  try {
    ...
    if (process.env.WORKING_ENV == "development") {
      ...
      redirectionUrlFrontendSUCCESS =
        process.env.REDIRECTION_SUCCESS_URL_FRONTEND_TESTING;
      redirectionUrlFrontendFAIL =
        process.env.REDIRECTION_FAIL_URL_FRONTEND_TESTING;
    } else {
      key = process.env.PHONEPAY_SALT_PRODUCTION;
      merchantId = process.env.PHONEPAY_MERCHANT_PRODUCTION;
      redirectionUrl = process.env.redirectUrlPhonePayProduction;
      ...
      redirectionUrlFrontendSUCCESS =
        process.env.REDIRECTION_SUCCESS_URL_FRONTEND_PRODUCTION;
      redirectionUrlFrontendFAIL =
    ...
    const updatedPayment = await PaymentModel.findById({
...
export const createPayment = asyncHandler(async (req, res) => {
  const { name, state, city, locality, auctionType, budget } = req.body;
